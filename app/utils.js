...
function estimatedSupply(height) {
	...
	if (height > halvingBlockInterval) {
        throw new Error(`Height ${height} exceeds halving block interval ${halvingBlockInterval}.`);
    }

	let blockRewardFunction = coinConfig.blockRewardFunction(0, global.activeBlockchain);
	let supply = new Decimal(height).times(blockRewardFunction);
...
function getVoutAddress(vout) {
	...
	if (vout && vout.scriptPubKey) {
        // First check if it's a pubkey type
        if (vout.scriptPubKey.type === "pubkey" && vout.scriptPubKey.desc) {
            // Extract the public key from the desc field
            // The format is "pk(public_key)#checksum"
            const match = vout.scriptPubKey.desc.match(/pk\((.*?)\)#/);
            if (match && match[1]) {
                return match[1];  // Return the public key
            }
